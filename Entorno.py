...
def peticion_gpt(word_1,word_2):
    ...
    embeddings = OpenAIEmbeddings(openai_api_key='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')

    docsearch1 = Pinecone.from_existing_index(index_name, embeddings)
    
    chain = load_qa_chain(llm, chain_type="stuff")
    query = f"Describa en formato de regla, 5 relaciones posible entre {word_1} y {word_2}. escribe cada relacion entre []"
    print(query)
    docs = docsearch1.similarity_search(query)
    print(chain.run(input_documents = docs, question = query))
    docs=chain.run(input_documents = docs, question = query)
    return docs
